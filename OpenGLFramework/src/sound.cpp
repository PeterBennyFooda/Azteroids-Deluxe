...
std::unordered_map<int, IXAudio2SourceVoice*> audios;
...
public:
    ...
    void Play(int soundId) {
        ...
        if (iter != mSounds.end()) {
            SoundSource& sound = (*iter).second;
            PlayAudio(pXAudio2, (WAVEFORMATEX*) &sound.wfx, &sound.buffer, soundId);
        }
    }
...
static HRESULT StopAudio(IXAudio2* pXAudio2, int soundId) {
    ...
    std::unordered_map<int, IXAudio2SourceVoice*>::iterator iter = audios.find(soundId);
    if (iter != audios.end()) 
        hr = (*iter).second->Stop();
